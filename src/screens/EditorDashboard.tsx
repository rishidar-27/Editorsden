...
export function EditorDashboard({ onNavigate }: EditorDashboardProps) {
  ...
  const activeCount = myAssignedTasks.filter((t) => t.status === 'In Progress' || t.status === 'Assigned').length;
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-[#f4f6fb] dark:bg-[#09090B] px-4 sm:px-6 lg:px-10 py-7 font-sans transition-colors text-gray-900 dark:text-zinc-100">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">
              You have <span className="font-bold text-gray-800 dark:text-zinc-200">{activeCount} {activeCount === 1 ? 'active task' : 'active tasks'}</span> currently assigned to you.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Card 1: Status */}
          <div className="bg-white dark:bg-zinc-900 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex items-center gap-4">
          ...
          {/* Card 2: Active Tasks */}
          <div className="bg-white dark:bg-zinc-900 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-zinc-800 text-gray-800 dark:text-zinc-200 flex items-center justify-center shrink-0">
              <FileText className="w-6 h-6 stroke-[2]" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-gray-400">Active Tasks</p>
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white leading-snug">{activeCount}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 bg-white rounded-2xl p-6 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
            ...
            <div className="space-y-3 pt-2">
              {myAssignedTasks.length === 0 ? (
                <div className="py-12 px-4 text-center rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-900/30">
                  <div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-zinc-800 text-gray-400 dark:text-zinc-500 flex items-center justify-center mx-auto mb-2.5">
                    <FileText className="w-5 h-5" />
                  </div>
                  <h4 className="text-xs font-bold text-gray-800 dark:text-zinc-200">No active deliverables assigned</h4>
                  <p className="text-[11px] text-gray-400 dark:text-zinc-500 mt-0.5 max-w-xs mx-auto">
                    When project managers assign you tasks or campaigns, they will show up here with live status and deadlines.
                  </p>
                </div>
              ) : (
          ...
          <div className="lg:col-span-4 bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-100 dark:border-zinc-800 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-5">
            {/* Header */}
            <div className="flex items-center justify-between pb-1">
              <div className="flex items-center gap-2">
                <UserIcon className="w-5 h-5 text-gray-900 dark:text-zinc-200 stroke-[2]" />
                <h3 className="font-bold text-gray-900 dark:text-white text-base">My Creator Profile</h3>
              </div>
              <button
                onClick={() => onNavigate('/editor/profile')}
                className="text-xs font-semibold text-gray-900 dark:text-zinc-200 hover:underline flex items-center gap-1 transition-colors"
              >
            ...
            <div className="flex items-center gap-3.5">
              <img
                ...
                alt={editor?.fullName || 'Creator'}
                className="w-14 h-14 rounded-full object-cover border border-gray-200 dark:border-zinc-700 shadow-2xs"
              />
              <div>
                <h4 className="font-bold text-gray-900 dark:text-white text-base leading-snug">
                  {editor?.fullName || 'Creator Profile'}
                </h4>
                <p className="text-xs text-gray-400 dark:text-zinc-500 flex items-center gap-1 mt-0.5">
                  <MapPin className="w-3 h-3 text-gray-400" />
                  <span>{editor?.city || 'Location not specified'}</span>
                </p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <span className="text-xs font-medium text-gray-400 dark:text-zinc-400 block">Editing Skills</span>
              <div className="flex flex-wrap gap-1.5">
                {editor?.skills && editor.skills.length > 0 ? (
                  editor.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 text-xs font-medium rounded-full"
                    >
                      {skill}
                    </span>
                  ))
                ) : (
                  <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-zinc-500 italic">
                    <span>No skills added yet.</span>
                    <button
                      onClick={() => onNavigate('/editor/profile')}
                      className="text-xs font-medium text-gray-900 dark:text-zinc-200 underline not-italic hover:opacity-80 cursor-pointer"
                    >
                      Add in Profile →
                    </button>
                  </div>
            ...
            <div className="space-y-1">
              <span className="text-xs font-medium text-gray-400 dark:text-zinc-400 block">Software Stack</span>
              {editor?.editingSoftware && editor.editingSoftware.length > 0 ? (
                <p className="text-xs font-semibold text-gray-700 dark:text-zinc-300 leading-relaxed">
                  {editor.editingSoftware.join(', ')}
                </p>
              ) : (
                <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-zinc-500 italic">
                  <span>No software stack specified.</span>
                  <button
                    onClick={() => onNavigate('/editor/profile')}
                    className="text-xs font-medium text-gray-900 dark:text-zinc-200 underline not-italic hover:opacity-80 cursor-pointer"
                  >
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-2.5 px-4 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-900 dark:text-white font-semibold text-xs sm:text-sm rounded-xl transition-colors flex items-center justify-center gap-1.5"
              >
