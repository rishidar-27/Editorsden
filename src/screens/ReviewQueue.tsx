...
export function ReviewQueue({ onNavigate }: ReviewQueueProps) {
  ...
  return (
    <div className="max-w-[1360px] mx-auto px-4 lg:px-6 py-6 space-y-6">
      ...
      {/* Review Queue Submission Cards Stack */}
      <div className="space-y-4">
        {submissions.map((item) => (
          <Card
            key={item.id}
            className="p-5 bg-white border border-gray-100 rounded-2xl shadow-2xs hover:shadow-md transition-all relative"
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center pr-6">
              ...
              <div className="lg:col-span-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h3 className="font-bold text-gray-900 text-sm truncate">{item.editorName}</h3>
                  <p className="text-xs text-gray-400 truncate mt-0.5">{item.campaign}</p>
              ...
              <div className="lg:col-span-3 border-t lg:border-t-0 lg:border-l border-gray-100 pt-3 lg:pt-0 lg:pl-6 space-y-2">
                <h4 className="font-bold text-gray-900 text-sm truncate">{item.taskTitle}</h4>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 bg-violet-50 text-violet-700 text-xs font-semibold rounded-full border border-violet-100">
                    {item.taskSkill}
                  </span>
                  <span className="px-2.5 py-0.5 bg-amber-50 text-amber-700 border border-amber-100 text-xs font-semibold rounded-full flex items-center gap-1">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs text-violet-600 font-semibold hover:underline mt-1 group"
                >
                  <span className="w-4 h-4 rounded bg-gradient-to-tr from-amber-500 via-emerald-500 to-indigo-600 flex items-center justify-center text-[8px] text-white font-bold shrink-0">
                    ▲
                  </span>
              ...
              <div className="lg:col-span-3 flex justify-center">
                <div className="relative w-44 h-24 rounded-xl overflow-hidden bg-gray-100 border border-gray-200 group cursor-pointer">
                  <img
                    src={item.thumbnail}
                    alt={item.taskTitle}
                  ...
                  <div className="absolute inset-0 bg-black/20 flex items-center justify-center group-hover:bg-black/30 transition-colors">
                    <div className="w-8 h-8 rounded-full bg-white/90 shadow-md flex items-center justify-center text-gray-900 group-hover:scale-110 transition-transform">
                      <Play className="w-4 h-4 fill-gray-900 ml-0.5" />
                    </div>
                  </div>
                  {item.duration && (
                    <span className="absolute bottom-1.5 right-1.5 px-1.5 py-0.5 bg-black/75 text-white text-[10px] font-medium rounded shadow-2xs">
                      {item.duration}
                    </span>
                  )}
                </div>
              ...
              <div className="lg:col-span-3 flex flex-col sm:flex-row lg:flex-col items-start lg:items-end justify-between gap-4">
                <div className="text-left lg:text-right space-y-1">
                  <div>
                    <span className="text-[10px] text-gray-400 font-medium block">Submitted</span>
                    <span className="text-xs font-bold text-gray-900">{item.submittedTime}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-gray-400 font-medium block">Due</span>
                ...
                <div className="flex flex-col gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold rounded-xl shadow-2xs transition-colors w-full sm:w-32"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-xs font-semibold rounded-xl shadow-2xs transition-colors w-full sm:w-32"
                  >
